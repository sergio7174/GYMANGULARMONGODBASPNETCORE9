  ...
  this.authService.getOneAdmin().pipe(
    takeUntilDestroyed(this.destroyRef)).subscribe({
      next: ( response:any ) => {
        ...
        alert("Estoy en login.component - line 80 - HaveAdmin:  " + this.HaveAdmin );

        if (this.HaveAdmin!='false') {
          this.haveAdmin = 'true';
        } else { this.haveAdmin = 'false'; }
      }}
  );
}
...
/**** my postdata new block *******************/
    ...
    postData.append('Username', this.registerForm.get('fullName')?.value);
    ...
    next: (response) => {
        ...
        this.newUser = this.dataUser.username;

        alert("Estoy en register.component - line 149 - this.username:  " + this.dataUser?.username);
