  ...
  this.memberService.createmember(postData).pipe(
   ...
   takeUntilDestroyed(this.destroyRef)).subscribe({
     next: (response) => {
              ...
              this.DataMemberBack = response;
              this.DataMember = this.DataMemberBack;
               
              // var to get if new member was created
              this.Newmember = this.DataMemberBack.member;
              // var to handle if there is an mistake in backend 
              const messageBackend = this.DataMember.messagemExist;



if (!this.Newmember) {     
  if (messageBackend){
    this.toast.error(messageBackend);
    //alert("Estoy en bill.component - line 167 - messageBackend:  " + messageBackend);
  }}

if (this.Newmember) {
this.toast.success('Create supply component - line 170 -Create Member successfully');
